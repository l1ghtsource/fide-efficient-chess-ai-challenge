...
void initTT(uint64_t megabytes) {
    ...
    // Start with a smaller keysize of 12 bits instead of 16
    // This allows for smaller table sizes than 2MB
    uint64_t keySize = 12ull;
    
    // // Find the largest keysize that is still within our given megabytes
    // while ((1ull << keySize) * sizeof(TTBucket) <= megabytes * MB) keySize++;
    // keySize--; // Step back one size to ensure we're under the limit
    
    // Simple allocation without huge pages, works on all platforms
